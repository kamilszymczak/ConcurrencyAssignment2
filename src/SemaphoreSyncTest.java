...
public class SemaphoreSyncTest {
    ...
    private Thread[] initP1Test(int initThreads){
        final SemaphoreSync semaphore = new SemaphoreSync(Phase.ONE);
        Thread threads[] = new Thread[initThreads];
        for (int i = 0 ; i < initThreads; i++){
            threads[i] = new Thread(new ThreadTester(semaphore));
            threads[i].start();
        }
        return threads;
    }
    ...
    void testPhase2a() {
        ...
        SemaphoreSync semaphore = new SemaphoreSync(Phase.TWO);
        Thread threads[] = new Thread[initThreads];

        // Even number required for this thread assignment strategy
        for (int i = 0 ; i < initThreads/2; i++){
            threads[i] = new Thread(new ThreadTester(semaphore, 0));
            threads[i+(initThreads/2)] = new Thread((new ThreadTester(semaphore, 1)));
            threads[i].start();
    ...
    void testPhase2c(int threadBound){
        ...
        for (int i = 0; i < initThreads; i++){
            ...
            threads[i] = new Thread(new ThreadTester(semaphore, gid));
    ...
    void testPhase2d(int threadBound){
        ...
        // Even number required for this thread assignment strategy
        for (int i = 0 ; i < initThreads; i++){
            threadG1[i] = new Thread(new ThreadTester(semaphore, 0));
            threadG2[i] = new Thread(new ThreadTester(semaphore, 1));
            threadG1[i].start();
            threadG2[i].start();
        }
    ...
    void testPhase3a(int threadBound){
        ...
        SemaphoreSync semaphore = new SemaphoreSync(Phase.THREE);
        Thread threadG1[] = new Thread[initThreads];
        Thread threadG2[] = new Thread[initThreads];
